...
public class PlayerController : MonoBehaviour
{
    ...
    private bool hasPerformedDashAttack = false; // Cờ để đảm bảo attack chỉ gọi 1 lần trong quá trình dash.
        ...
        [Header("Stomp Attack")] // <<< MỤC MỚI

        [Tooltip("Lực nảy lên khi người chơi dậm lên đầu kẻ địch.")]

        public float stompJumpForce = 10f;

        [Tooltip("Sát thương gây ra khi dậm lên kẻ địch.")]
        ...
        private Collider2D groundCollider; // <<< BIẾN MỚI: Lưu trữ thông tin về collider mà người chơi đang đứng trên.
...

        {
            ...
            // <<< THAY ĐỔI: Lưu lại collider chạm đất thay vì chỉ lưu true/false

            groundCollider = Physics2D.OverlapCircle(groundCheck.position, groundCheckRadius, layerMask: groundLayer);
...

        {
...

            {
                ...
                // <<< LOGIC MỚI: Kiểm tra xem có đang đứng trên Enemy không >>>

                                Enemy enemy = groundCollider.GetComponent<Enemy>();

                                if (enemy != null)

                                {

                                    // Thực hiện dậm nhảy và gây sát thương

                                    StompJump();

                                    enemy.TakeDamage(stompDamage, transform);
        ...
        // <<< HÀM MỚI: Tách logic nhảy thường và dậm nhảy >>>
        ...
        private void StompJump()

        {

            // Dậm nhảy có thể không tốn stamina, hoặc tốn ít hơn. Ở đây tôi cho là không tốn.
            ...
            rb.AddForce(jumpDirection * stompJumpForce, ForceMode2D.Impulse);
...

        {
            ...
            if (GrafityDown) { rb.linearVelocity = new Vector2(rb.linearVelocity.x, 0); return Vector2.up; }

            if (GrafityUp)   { rb.linearVelocity = new Vector2(rb.linearVelocity.x, 0); return Vector2.down; }

            if (GrafityLeft) { rb.linearVelocity = new Vector2(0, rb.linearVelocity.y); return Vector2.right; }

            if (GrafityRight){ rb.linearVelocity = new Vector2(0, rb.linearVelocity.y); return Vector2.left; }
